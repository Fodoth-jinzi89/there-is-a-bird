...
import keletu.guaniao.content.bird.nightheron.NightHeronLandingSelector;
...
extends Goal {
    ...
    private Response chooseResponse() {
        Player player;
        Player player2 = player = this.threat != null ? this.threat : this.findNearestRelevantPlayer();
        if (player == null && !this.nightHeron.hasExternalFright()) {
            return Response.NONE;
        }
        if (player == null) {
            return this.nightHeron.hasSevereExternalFright() || this.nightHeron.getRecentFrightCount() >= 3 ? Response.LONG_FLIGHT : Response.RUN;
        }
        double distance = Math.sqrt(this.nightHeron.distanceToSqr((Entity)player));
        boolean tempted = this.nightHeron.isTemptingPlayer(player);
        if (tempted && !player.isSprinting() && distance > 3.5) {
            return Response.NONE;
        }
        double adjustedDistance = distance + (tempted ? 3.0 : 0.0);
        if (!tempted && this.nightHeron.getRecentFrightCount() >= 3 && adjustedDistance <= 15.0) {
            return Response.LONG_FLIGHT;
        }
        if (adjustedDistance <= 3.0 && !tempted) {
            return Response.LONG_FLIGHT;
        }
        if (player.isSprinting() && adjustedDistance <= 8.0) {
            return Response.LOW_FLIGHT;
        }
        if (this.nightHeron.getBehaviorState() == NightHeronBehaviorState.ROOSTING && adjustedDistance <= 6.0) {
            return Response.LOW_FLIGHT;
        }
        if (adjustedDistance <= 4.25 && (!tempted || distance <= 2.5)) {
            return Response.LOW_FLIGHT;
        }
        if (adjustedDistance <= 6.0 || this.nightHeron.horizontalCollision) {
            return Response.RUN;
        }
        if (adjustedDistance <= 9.0) {
            return Response.WALK;
        }
        if (adjustedDistance <= 13.0) {
